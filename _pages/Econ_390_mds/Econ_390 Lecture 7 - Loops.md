...
layout: single
sitemap: false
permalink: /pages/Econ_390_SP26_L7_teaching/
---
...
```
    ...
    GDP is the 0 entry
    POP is the 1 entry
    INVEST is the 2 entry
    EXPORTS is the 3 entry
...
for i, name in enumerate(var_names, start=1):
    if i == 1:    
        ...
        print(name, " is the ", i, "nd entry", sep="")
    elif i == 3:
        print(name, " is the ", i, "rd entry", sep="")
    else:
        print(name, " is the ", i, "th entry", sep="")
```

    GDP is the 1st entry
    POP is the 2nd entry
    INVEST is the 3rd entry
...
```
    ...
    <zip object at 0x000001F6EA40D940>
...
x = "go"
print('Type stop to end this madness:')
while x != "stop":
    x = input()
```

    Type stop to end this madness:
...
years_ago = []
for i in range(4):
     years_ago.append(year-i)
[years_ago_1, years_ago_2, years_ago_3] = years_ago[1:4]
# 2.
for country in inflation_dict:
    if inflation_dict[country] <= 2:
        print("{c} recently experienced low inflation of {pi}%".format(c=country, pi=inflation_dict[country]))
    elif inflation_dict[country] > 2 and inflation_dict[country] <= 50:
        print("{c} recently experienced high inflation of {pi}%".format(c=country, pi=inflation_dict[country]))
    else:
        print("{c} recently experienced hyper inflation of {pi}%".format(c=country, pi=inflation_dict[country]))
```
...
```python
# Same with strings
names = ['Maddox', 'Pax', 'Zahara', 'Shiloh', 'Knox', 'Vivienne']
