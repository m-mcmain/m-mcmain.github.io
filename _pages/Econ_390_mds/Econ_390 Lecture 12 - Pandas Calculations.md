...
layout: single
sitemap: false
permalink: /pages/Econ_390_SP26_L12_teaching/
---
...
```python
# Read in df
df = pd.read_stata('FUSION_annual_1995_EMP.dta')
df
```
...
```python
# Or set the index
ENIA_1995 = pd.read_stata('FUSION_annual_1995_EMP.dta', index_col = 'NUI')
ENIA_1995
```
...
```
    ...
    13.8 ms ± 1.75 ms per loop (mean ± std. dev. of 7 runs, 100 loops each)
...
```
    ...
    159 μs ± 28.3 μs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
...
ENIA_1995['avg_wages_dollar'] = 1000*ENIA_1995['total_emps']/ENIA_1995['total_wages_dollar']
ENIA_1995
```
...
```python
# Another way
ENIA_1995_emps.sum(axis='rows').sum()
```
...
```python
# Notice patterns
ENIA_1995_cleaned.loc[:,ENIA_1995_cleaned.columns.str.startswith('TO')].sum(axis='columns')
```
...
# skipna option
ENIA_1995_cleaned.loc[:,ENIA_1995_cleaned.columns.str.startswith('TO')].sum(axis='columns')
ENIA_1995_cleaned.loc[:,ENIA_1995_cleaned.columns.str.startswith('TO')].sum(axis='columns', skipna=False)
```
...
ENIA_1995_cleaned.loc[:,ENIA_1995_cleaned.columns.str.startswith('TOT')].sum(axis='columns', skipna=False)
```
...
print(ENIA_1995_cleaned.iloc[:, 2:].mean(axis='columns'),'\n')
# And rows
print(ENIA_1995_cleaned.mean(axis='rows'),'\n')
# Or make a series
print('Average Women Contracted:',ENIA_1995_cleaned.loc[:,'TOTMUJ'].mean(),'\n')
# Or take the mean twice to get the entire mean of the table - even though it doesn't mean anything in this context!
print('Everything I guess:',ENIA_1995_cleaned.mean(axis='rows').mean())
```
    ...
    TOTHOM                    73.540000
    TOTMUJ                    24.829545
    REMEMP                139761.410526
    REGEMP                 19624.164179
    REMCOM                101002.416667
    REGCOM                  1164.875000
    REMOBR                126966.591837
    REGOBR                 22021.600000
    TOHSC                      3.789474
    TOMSC                      5.166667
    EXPORTADOS                 1.000000
    ...
    avg_wages_dollar         255.554033
    dtype: float64 
    
    Average Women Contracted: 24.829545454545453 
    
    Everything I guess: 41884.64000506775
    


```python
# Maximum
ENIA_1995_cleaned.max(axis='rows')
```
...
```python
# Location of the max
ENIA_1995_cleaned.idxmax(axis='rows')
```
    ...
    total_emps            11514
    total_wages           12860
    total_wages_dollar    12860
    avg_wages_dollar      15045
    dtype: int32
...
```python
# Cumulative Sum
ENIA_1995_cleaned.cumsum(axis='rows')
```
...
```
    ...
    0            17.930496         45.396945  
    1            20.029737         47.298024  
    2            21.894610         55.060390  
    3            23.310904         73.545378  
    4            27.486706         76.668004  
    ..                 ...               ...  
    95         4767.657065        613.479972  
    96         5224.225700        639.426796  
    97         6862.584108        643.475676  
    98         8209.168116        678.857374  
    99         9633.290492       1027.070304  
...
```python
# Cumulative sum
pd.crosstab(ENIA_1995['REGION'],'cumulative', normalize='columns').cumsum()
```
...
tab['cumulative'] = tab['percent'].cumsum()
tab
```
...
ENIA_1995["emp_type"] = pd.cut(ENIA_1995["total_emps"], bins=[1,25,100,250,1000])
print(pd.crosstab(ENIA_1995["emp_type"], ENIA_1995["EXPORTADOS"]))
# 3.
print(pd.crosstab(ENIA_1995["emp_type"], ENIA_1995["EXPORTADOS"], normalize = "index"))
print(pd.crosstab(ENIA_1995["emp_type"], ENIA_1995["EXPORTADOS"], normalize = "columns"))
print(pd.crosstab(ENIA_1995["emp_type"], ENIA_1995["EXPORTADOS"], normalize = "all"))
```
    ...
    EXPORTADOS        0.0       1.0
    emp_type                       
    (1, 25]      0.975000  0.025000
    (25, 100]    0.810811  0.189189
    (100, 250]   0.538462  0.461538
    (250, 1000]  0.100000  0.900000
    EXPORTADOS        0.0       1.0
    emp_type                       
    (1, 25]      0.506494  0.043478
    (25, 100]    0.389610  0.304348
    (100, 250]   0.090909  0.260870
    (250, 1000]  0.012987  0.391304
    EXPORTADOS    0.0   1.0
    emp_type               
    (1, 25]      0.39  0.01
    (25, 100]    0.30  0.07
